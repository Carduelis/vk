...
$( ".element" ).draggable();
  $('#container section > div').addClass('active');
  $('#container section > div.active').bind('click', function() {
        currentIdSideOfBlock = $(this).attr('id');
        //alert(currentIdSideOfBlock);
...
function element(typeOfControl, i) {
  return '<div class="element" type="'+typeOfControl+'" num="'+i+'">  <div class="overlay"></div><div class="body"></div>  <div class="control"><a href="javascript:void(0);" class="click"></a></div></div>'
}

$( "#container section>div" ).droppable({
 hoverClass: "hover",
...
function select(typeOfElement) {
  ...
  if (typeOfElement == 'knob'){
    options = {
        ...
        view: startHtmlTags+'введите особый тип отображения</label><input data="view" type="text" value=""></div>'
    }
    var max = 12;
    for (var i = 0; i < max; i++) {
  ...
  if (typeOfElement == 'toggler'){
    options = {
        ...
        view: startHtmlTags+'или введите особый тип отображения</label><input data="view" type="text" value="">'
    }
    var max = 3;
    for (var i = 0; i < max; i++) {
       options.defaultPosition += '<option value="'+i+'">'+i+'</option>';
    }
       options.view += '<option>Выбрать</option><option value="simple">Двухпозиционный</option><option value="rotate">Поворотный</option><option value="complicated">Трехпозиционный</option>';
       options.view2 += '<option>Выбрать</option><option value="vert">Вертикально</option><option value="hor">Горизонтально</option>';
  }
  if (typeOfElement == 'lamp') {
    ...
    var max = 2;  
    options.size += '<option>Выбрать</option><option value="big">Обычная</option><option value="small">Небольшая</option>';
    options.defaultPosition += '<option>Выбрать</option><option value="off">Не горит</option><option value="on">Горит</option>';
    options.color += '<option>Выбрать</option><option value="yellow">Желтая</option><option value="green">Зеленая</option><option value="red">Красная</option>';
  }
  if (typeOfElement == 'rotator') {
    ...
    options = {
        ...
        view2: startHtmlTags+'Отображать цифры</label><select type="view2"><option>Выбрать</option><option value="yes">Да</option><option value="no">Нет</option></select></div>',
        view: startHtmlTags+'введите особый тип отображения</label><input data="view" type="text" value=""></div>'
    }
    var max = 2;
    for (var i = 0; i < max; i++) {
  ...
  if (typeOfElement == 'socket'){
    ...
    options.typeOfSocket += '<option>Выбрать</option><option value="one">Одно</option><option value="two">Два</option><option value="four">Четыре</option>';
  }
  if (typeOfElement == 'scale') {
    endHtmlTags = '';
    options = {
        typeScale: startHtmlTags+'Ввести название файла шкалы</label><option>Выбрать</option><input data="typeScale" type="text" value=""></div>',
        zeroPosition: startHtmlTags+'Позиция нуля</label><select data="zeroPosition"><option>Выбрать</option><option value="left">Слева</option><option value="center">По центру</option></select></div>',
        size: startHtmlTags+'Размер</label><select data="size"><option>Выбрать</option><option value="min">Небольшой</option><option value="mid">Средний</option><option value="max">Большой</option></select></div>',
    }
  }
